...
fstream filetext("text.txt");

if (filetext.is_open())
	{
		while (filetext>>temptext)
		{
...
fstream filenum("num.txt");

if (filenum.is_open())
	{
		while (filenum>>tempnum)
		{
...
ofstream fileoutprova;
fileoutprova.open("prova.txt");

if (fileoutprova.is_open())
{
	for (int i=0; i<righe; i++)
	fileoutprova << "ATOM" << "    " << num.at(3*i) << "    " << "CA" << "    " << text.at(i) << "    " << "A" << "    " << num.at(3*i+1)  << "    " << res[i].x << "    " << res[i].y << "    " << res[i].z << "    " << 0.75 << "    " << bfact.at(i) << "    " << "C"<< endl;
}
else 
{
...
//Costruisce la matrice di dimensione NxN con elementi le sottomatrici 3x3 date dalla struttura sotmat
double gamma=0.0L;
double raggioc=0.0L;
int var=0;
double R0=0.0L;
...
//Legge gli estremi del loop da fissare con una molla aggiuntiva
/*int bloc1=0; 
int bloc2=0;
cout << "Inserire un estremo del loop da fissare: ";
cin >> bloc1;
cout << "Inserire l'altro estremo del loop da fissare: ";
cin >> bloc2;
cout << endl;*/

raggioc=20.0; //Valore del raggio di cutoff 
...
for (int i=0; i<righe; i++)
{
	for (int j=0; j<righe; j++)
	{
		...
		var=i-j;
		//if((i==bloc1&&j==bloc2)||(i==bloc2&&j==bloc1)) //Chiude il loop con una molla
		//{
		//	gamma=-10.0L;
		//}
		//else
...
//Crea una matrice 3Nx3N nellambiente Eigen e copia la matrice di sottomatrici
...
//Calcola autovalori e autovettori tramite il pacchetto Eigen sfruttando il fatto che la matrice è simmatrica e reale e quindi autoaggiunta
SelfAdjointEigenSolver<MatrixXd> s(W);
s.compute(W);

//Scrive gli autovalori in un file
ofstream fileout7;
...
//Somma i deltaRik su k tralasciando i primi sei autovalori
...
//Produce il file con il profilo di mobilità (n° residuo, deltaRi, B-factor)
//Calcola le aree per la normalizazione
double area1=0.0L;
double area2=0.0L;
...
ofstream fileout2;
fileout2.open("mobilita.txt");

if (fileout2.is_open())
{
	for (int i=0; i<righe; i++)
	{
		fileout2 << i << " " << (area1/area2)*(deltaRi.at(i)) << " " << bfact.at(i) << endl;
	}
}
else 
{
...
//Calcola i gradi di collettività
double norm=0.0L;
double somma=0.0L;
double alfa=0.0L;
...
//Produce file con i gradi di collettività (n° residuo, kappa)
ofstream fileout9;
...
cout << "Ho calcolato il linking number per la configurazione base per i due loop: G(1-50/51-122)=" << linknumb(righe, i1, i2, j1a, j2a, res) << ", G(10-50/51-122)=" << linknumb(righe, i1, i2, j1b, j2b, res) << endl;

//Genera le configurazioni deformate di prova
vector <pos> def(righe);
double smax=0.0L;
cout << "Inserire il valore di s_max: ";
cin >> smax;
cout << endl;
int modo=0;
cout << "Inserire numero del modo lungo il quale modificare la proteina: ";
cin >> modo;
cout << endl;
...
//File per i valori
ofstream fileout777;
fileout777.open("valori.txt");

fileout777 << "Ho calcolato la deformazione di prova con s_max=" << smax << " per il modo k=" << modo << " ottenendo una differenza fra la distanza cm-res157 e cm-res157 deformati del " << 100*(abs(distk-dist))/dist << "% della distanza cm-res157" << endl;
cout << "Percentuale: " << 100*(abs(distk-dist))/dist << endl;
//Fa variare s nell'intervallo [-smax;smax] e calcola i linking number per ogni s per i due loop
...
//Stampa il file per i grafici di G e d
ofstream fileout999;
...
//Calcolo delle derivate di G' (divise per il valore della funzione in zero)
double delta=0.0;
delta=0.00001;

double Gap=0.0, Gbp=0.0, Gam=0.0, Gbm=0.0, dGa=0.0, dGb=0.0, d2Ga=0.0, d2Gb=0.0, Ga0=0.0, Gb0=0.0;
...
fileout777 << "dG (1-50) = " << dGa/(sqrt(pow(Ga0,2))) << endl; 
fileout777 << "dG (10-50) = " << dGb/(sqrt(pow(Gb0,2))) << endl;
fileout777 << "d2G (1-50) = " << d2Ga/(sqrt(pow(Ga0,2))) << endl;
fileout777 << "d2G (10-50) = " << d2Gb/(sqrt(pow(Gb0,2))) << endl;

//Calcolo la derivata della distanza (divise per il valore della funzione in zero)
...
fileout777 << "dd (51-122) = " << (a*b+c*d+e*f)/(a*a+c*c+e*e) << endl;
fileout777 << "d2d (51-122) = " << (b*b+d*d+f*f)/(a*a+c*c+e*e)-((a*b+c*d+e*f)*(a*b+c*d+e*f))/(pow(a*a+c*c+e*e,3)) << endl;
...
fileout777 << "dd (10-50) = " << (a*b+c*d+e*f)/(a*a+c*c+e*e) << endl;
fileout777 << "d2d (10-50) = " << (b*b+d*d+f*f)/(a*a+c*c+e*e)-((a*b+c*d+e*f)*(a*b+c*d+e*f))/(pow(a*a+c*c+e*e,3)) << endl;
