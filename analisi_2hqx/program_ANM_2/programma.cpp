...
struct pos {double x=0.0; double y=0.0;	double z=0.0; double bfact=0.0;};
struct sotmat {double H1=0.0; double H2=0.0; double H3=0.0;	double H4=0.0; double H5=0.0; double H6=0.0; double H7=0.0;	double H8=0.0; double H9=0.0;};
...
cout << "Inserire il valore (int) della distanza di cutoff: ";
cin >> raggioc;
cout << endl;
...
if (fileout13.is_open())
{
	for(int i=0; i<(3*righe); i++)
		...
		for(int k=0 k<(3*righe); k++)
			{
			fileout13 << s.eigenvectors().col(k)[i] << " ";
			}
		fileout13 << endl;
		}		
}
